...
def _companies_house_book_period(company):
    """Use the live Companies House accounts window, not a 31 March tax year."""
    filing = None
    try:
        from app import live_companies_house_filing
        filing = live_companies_house_filing(company, force=False)
    except Exception:
        filing = None
    src = filing or {}
    start = str(src.get('accounts_period_start') or (company or {}).get('accounts_period_start') or '')[:10]
    end = str(src.get('accounts_made_up_to') or (company or {}).get('accounts_made_up_to') or '')[:10]
    inc = str(src.get('inc') or src.get('inc_date') or (company or {}).get('inc_date') or '')[:10]
    last = str(src.get('accounts_last_made_up_to') or (company or {}).get('accounts_last_made_up_to') or '')[:10]
    if len(start) < 10 or len(end) < 10:
        return None
    is_first = 1 if (src.get('is_first_accounts') if src.get('is_first_accounts') is not None else not last) else 0
    return {
        'period_start': start,
        'period_end': end,
        'prior_period_start': None,
        'prior_period_end': None,
        'is_first_accounts': is_first,
        'incorporation_date': inc or None,
    }


def _default_period_from_company(company):
    ch = _companies_house_book_period(company)
    if ch:
        return ch
    inc = _parse_date(company.get('inc_date')) or datetime.date(2024, 4, 1)
    # Fallback only if Companies House has not given a made-up-to date.
...
def choose_path(company_id, data, user_id=None):
    ...
    period = _companies_house_book_period(company)
    if period:
        execute_db(
            """
            UPDATE ledger_books
            SET period_start=?, period_end=?, is_first_accounts=?,
                incorporation_date=COALESCE(?, incorporation_date),
                prior_period_start=NULL, prior_period_end=NULL,
            ...
            (
                period['period_start'],
                period['period_end'],
                period['is_first_accounts'],
                period.get('incorporation_date'),
                book['id'],
            ),
        )
    if kind == 'dormant':
        book = _book_for_company(company_id)
        _ensure_subscriber_share_capital(book, company)
    return workspace(company_id, user_id)


def _ensure_subscriber_share_capital(book, company):
    """Post unpaid subscriber shares from the Companies House statement of capital."""
    if not book or not book.get('id'):
        return
    already = query_db(
        "SELECT id FROM ledger_journals WHERE book_id = ? AND reference = ?;",
        (book['id'], 'CH-SH01'),
        one=True,
    )
    if already:
    ...
    capital = None
    try:
        from app import live_companies_house_filing, fetch_companies_house_share_capital
        filing = live_companies_house_filing(company, force=False) or {}
        capital = filing.get('share_capital')
        if not capital:
            capital = fetch_companies_house_share_capital((company or {}).get('company_number'))
    except Exception:
        capital = (company or {}).get('share_capital_gbp')
    try:
    ...
    if capital <= 0:
        capital = 1.0
    day = (book.get('period_end') or book.get('incorporation_date') or (company or {}).get('inc_date') or '')[:10]
    _write_journal(
        book['id'],
        day,
        'Subscriber share capital from Companies House statement of capital (unpaid)',
        [
            ('0100', capital, 0, 'Called up share capital not paid'),
            ('3000', 0, capital, 'Share capital'),
        ],
        reference='CH-SH01',
    )
...
def _ch_years(inc):
    ...
    years = []
    start = inc
    end = _last_day(inc.year + 1, inc.month)
    for index in range(6):
        years.append((start, end))
        start = end + datetime.timedelta(days=1)
        end = _last_day(end.year + 1, end.month)
    return years


def _window_for_statement(book, first_date, last_date):
    first = _parse_date(first_date)
    last = _parse_date(last_date)
    years = _ch_years((book or {}).get("incorporation_date"))
    if first and last and years:
        overlap = [row for row in years if row[0] <= last and row[1] >= first]
        inside = [row for row in overlap if first <= row[1] <= last]
        picked = (inside[0] if inside else (overlap[-1] if overlap else years[0]))
        return picked[0], picked[1]
    start = _parse_date((book or {}).get("period_start"))
    end = _parse_date((book or {}).get("period_end"))
    return start, end

def _movement_maps(book_id):
    book = query_db("SELECT period_start, period_end FROM ledger_books WHERE id = ?;", (book_id,), one=True) or {}
    start = str(book.get("period_start") or "")[:10]
    end = str(book.get("period_end") or "")[:10]
    extra = ""
    params = [book_id]
    if start and end:
        extra = " AND date(j.journal_date) >= date(?) AND date(j.journal_date) <= date(?)"
...
def categorise_line(description, amount):
    ...
    lowered = text.lower()
    amt = money(amount)
    if amt == 0 or re.search(r'\bopening balance\b', lowered):
        return None
    for pattern, code, label in CATEGORY_RULES:
        if re.search(pattern, lowered):
            return {'code': code, 'label': label, 'confidence': 'high', 'needs_review': 0}
    if amt > 0:
        return {'code': '4000', 'label': 'Sales', 'confidence': 'high', 'needs_review': 0}
    if amt < 0:
        return {'code': '7900', 'label': 'General Expenses', 'confidence': 'high', 'needs_review': 0}
    return None
...
def import_statement(company_id, data, user_id=None, files=None):
    ...
    win_import_start, win_import_end = _window_for_statement(book, first_date, last_date)
    imported = 0
    skipped = 0
    for index, line in enumerate(lines):
        ...
        if digest in existing:
            skipped += 1
            continue
        txn_day = _parse_date(line.get("txn_date"))
        if win_import_end and txn_day and txn_day > win_import_end:
            skipped += 1
            continue
        if win_import_start and txn_day and txn_day < win_import_start:
            skipped += 1
            continue
    ...
    win_start, win_end = _window_for_statement(current or book, first_date, last_date)
    if current and not current.get('sample_loaded'):
        in_window = [_parse_date(row.get("txn_date")) for row in lines]
        in_window = [day for day in in_window if day and (not win_start or day >= win_start) and (not win_end or day <= win_end)]
        first_in = min(in_window) if in_window else _parse_date(first_date)
        start = first_in or win_start
        end = win_end or _parse_date(last_date)
        updates = []
        values = []
        if start:
...
def _bank_payload(book, accounts=None, company_id=None, user_id=None):
    ...
    period_start = str(book.get("period_start") or "")[:10]
    period_end = str(book.get("period_end") or "")[:10]
    if period_start and period_end:
        lines = query_db(
            ...
            FROM ledger_bank_lines
            WHERE book_id = ? AND date(txn_date) >= date(?) AND date(txn_date) <= date(?)
            ORDER BY txn_date, id;
            """,
            (book['id'], period_start, period_end),
        ) or []
    else:
        lines = query_db(
            """
