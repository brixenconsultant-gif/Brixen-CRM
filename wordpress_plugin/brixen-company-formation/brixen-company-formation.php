...
class Brixen_Company_Formation_Form {
    ...
    private function collect_posted_fields($product_id) {
        ...
        return array(
            ...
            'email_password' => sanitize_text_field(wp_unslash($_POST['email_password'] ?? '')),
        ...
        <div class="cfs-form-container" id="ivs-form-wrapper">
            <form id="cfs-formation-form" method="post" enctype="multipart/form-data">
                <div class="cfs-form-section">
                    ...
                    <div class="cfs-form-row">
                        <div class="cfs-form-col">
                            <label class="cfs-label">Desired Company Name <span class="cfs-required">*</span></label>
                            <input type="text" name="company_name" class="cfs-input<?php echo $company_locked ? ' cfs-input-locked' : ''; ?>" value="<?php echo esc_attr($company_name); ?>" placeholder="The company name you want to register" required <?php echo $company_locked ? 'readonly' : ''; ?>>
                            <p class="cfs-hint"><?php echo $company_locked
                                ? 'This is filled automatically from the name you searched and cannot be changed.'
                                : 'This company is not registered yet. Enter the exact name you want Companies House to create.'; ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="cfs-form-section">
                    ...
                    <div class="cfs-form-row">
                        <div class="cfs-form-col">
                            <label class="cfs-label">Nationality <span class="cfs-required">*</span></label>
                            <input type="text" name="nationality" class="cfs-input" placeholder="e.g. British / Pakistani" value="<?php echo esc_attr($this->sv($saved, 'nationality')); ?>" required>
                        </div>
                        <div class="cfs-form-col">
                    ...
                    <div class="cfs-form-row">
                        <div class="cfs-form-col">
                            <label class="cfs-label">Email <span class="cfs-required">*</span></label>
                            <input type="email" name="registered_email" class="cfs-input" placeholder="Enter your email" value="<?php echo esc_attr($this->sv($saved, 'registered_email')); ?>" required>
                        </div>
                        <div class="cfs-form-col">
                            <label class="cfs-label">Email Password <span class="cfs-required">*</span></label>
                            <input type="text" name="email_password" class="cfs-input" placeholder="Mailbox password for this email" value="<?php echo esc_attr($this->sv($saved, 'email_password')); ?>" required autocomplete="off">
                            <p class="cfs-hint">Needed so our team can access the company mailbox while forming the company.</p>
                        </div>
                    </div>
                </div>
    ...
    public function enqueue_assets() {
        ...
        wp_register_style('cfs-frontend', false, array(), '1.1.8');
        wp_enqueue_style('cfs-frontend');
        wp_add_inline_style('cfs-frontend', $this->css() . $this->account_menu_css() . $this->bank_offer_css());
        wp_register_script('cfs-frontend', false, array('jquery'), '1.1.8', true);
        wp_enqueue_script('cfs-frontend');
        // Hard rule: My Account → Log in / Client Portal always stays on the website client panel.
        // Never use portal.brixenconsultants.com or wp-login.php for these header links.
        $account_url = home_url('/client-panel/');
        if (function_exists('wc_get_page_permalink')) {
            $wc_account = wc_get_page_permalink('myaccount');
            if ($wc_account) {
                $account_url = $wc_account;
            }
        }
        $account_url = $this->force_website_client_panel_url($account_url);
    ...
    /**
     * Reject CRM portal / wp-login targets for storefront account links.
     */
    private function force_website_client_panel_url($url) {
    ...
    public function process_form() {
        ...
        if (($form_data['nationality'] ?? '') === '' || ($form_data['email_password'] ?? '') === '' || ($form_data['uk_contact_number'] ?? '') === '') {
            wp_send_json_error(array('message' => 'Nationality, email password and UK phone number are required'));
        }
    ...
    public function save_form_data_to_order($order_id) {
        ...
        $note .= 'Nationality: ' . ($form_data['nationality'] ?? '') . "\n";
...
jQuery(function($){
    ...
    var websitePanel = '/client-panel/';
    function forceWebsiteClientPanel(url) {
        var u = String(url || '').trim();
        if (!u) return websitePanel;
        if (/portal\.brixenconsultants\.com/i.test(u)) return websitePanel;
        if (/wp-login\.php/i.test(u)) return websitePanel;
        return u;
    }
    var loginUrl = forceWebsiteClientPanel(cfg.loginUrl || cfg.accountUrl || websitePanel);
    var portalUrl = forceWebsiteClientPanel(cfg.portalUrl || cfg.accountUrl || loginUrl);
    ...
    function enhance($a){
        ...
        // My Account itself must stay on website client panel.
        ...
        if (loggedIn) {
            $menu.append('<li role="none"><a role="menuitem" class="brixen-website-client-panel" href="' + portalUrl + '"><i class="fas fa-user-circle" aria-hidden="true"></i> Client Portal</a></li>');
            $menu.append('<li role="none"><a role="menuitem" href="' + logoutUrl + '"><i class="fas fa-sign-out-alt" aria-hidden="true"></i> Logout</a></li>');
        } else {
            $menu.append('<li role="none"><a role="menuitem" class="brixen-website-client-panel" href="' + loginUrl + '"><i class="fas fa-sign-in-alt" aria-hidden="true"></i> Log in</a></li>');
        }
        $('body').append($menu);

        // Hard block: never navigate Log in / Client Portal to the CRM host.
        $menu.on('click', 'a.brixen-website-client-panel', function(e){
            var href = forceWebsiteClientPanel($(this).attr('href'));
            $(this).attr('href', href);
            if (/portal\.brixenconsultants\.com/i.test(String($(this).attr('href') || ''))) {
                e.preventDefault();
                window.location.href = websitePanel;
            }
        });
